...
import type {
	MonthlyWeightPhrInfo,
	MonthlyWeightPhrItem,
	VisiblePHRDetails,
...
const Detail = ({ infos, isVisible }: DetailProps) => {
	...
	return (
		<div className={isVisible ? 'bg-bg-gray flex flex-col gap-5 p-5 rounded-[1.25rem]' : 'hidden'}>
			{infos.map(({ bmi = 0, day, height = 0, weight = 0 }, i) => (
				<div className="flex items-center justify-between" key={`month-weight-detail-${i}`}>
