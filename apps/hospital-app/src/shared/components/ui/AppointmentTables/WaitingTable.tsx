		...
		() => [
			{
				...
				cell: ({ getValue }) => {
					if (getValue<AppointmentType>() === 'sdn') {
						return (
							<span className="font-normal leading-normal text-base text-primary-70">
								빠른 진료
							</span>
						);
					}
					return (
						<span className="font-normal leading-normal text-base text-text-100">
							일반 진료
						</span>
					);
				},
			...
			},
			{
				...
				cell: ({ getValue }) => {
					return (
						<span className="block font-normal leading-normal text-base text-text-100 truncate">
							{getValue<string>()}
						</span>
					);
				},
