...
export default function CompletedConsultationDetail() {
  ...
  // Use mock data for now since backend doesn't have all fields yet
  const consultationData = mockCompletedConsultationDetails[id || '1'] || mockCompletedConsultationDetails['1'];

  // Override with API data where available
  if (appointment) {
    consultationData.appointmentNumber = appointment.externalId;
    consultationData.treatment.symptoms = appointment.symptoms;
    consultationData.treatment.symptomImages = appointment.symptomImages;
    if (appointment.endedAt) {
      consultationData.completedAt = format(new Date(appointment.endedAt), 'dd/MM/yyyy HH:mm');
    }
  }
