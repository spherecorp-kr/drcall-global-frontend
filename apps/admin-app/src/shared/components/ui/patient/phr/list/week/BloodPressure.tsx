...
import type {
	VisiblePHRDetails,
	WeeklyPressurePhrInfo,
	WeeklyPressurePhrItem,
...
const Detail = ({ infos, isVisible }: DetailProps) => {
	return (
		<div className={isVisible ? 'bg-bg-gray flex flex-col gap-5 p-5 rounded-[1.25rem]' : 'hidden'}>
			{infos.map(({ diastolic = 0, pulse = 0, systolic = 0, time }, i) => (
				<div className='flex items-center justify-between' key={`week-pressure-detail-${i}`}>
